...
edge InserInForest(int u,int v, int wt)
{
    edge e;
    e.u=u;
    e.v=v;
...
forest Prim(graph g, int **w)
{
    ...
    forest A = (forest)malloc((g.size+1)*sizeof(edge));
    int u,v,index=0;
    NODE curr;
    while(p.size>0)
    {
...
int main()
{
    ...
    g = AddEdgeW(g,7,4,5,&w);
    g = AddEdgeW(g,8,3,1,&w);
    g = AddEdgeW(g,4,2,2,&w);
    g = AddEdgeW(g,3,4,1,&w);
    g = AddEdgeW(g,3,5,8,&w);
    g = AddEdgeW(g,6,5,2,&w);
    g = AddEdgeW(g,6,3,4,&w);
    g = AddEdgeW(g,2,8,7,&w);
    g = AddEdgeW(g,7,3,9,&w);
    NODE curr = g.list[1].top;
    forest A = Prim(g,w);
    PrintForest(A,g.size);
    return 0;
